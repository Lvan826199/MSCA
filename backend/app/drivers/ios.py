...
class IOSDriver(AbstractDeviceDriver):
    ...
    async def get_screenshot(self) -> bytes:
        ...
        try:
            async with aiohttp.ClientSession() as http:
                async with http.get(f"{base}/screenshot") as resp:
                    if resp.status == 200:
    ...
    async def _get_window_size(self) -> dict:
        ...
        session_path = f"/session/{self._session_id}" if self._session_id else ""
        async with aiohttp.ClientSession() as http:
            async with http.get(f"{base}{session_path}/window/size") as resp:
                data = await resp.json()
    ...
    async def _read_mjpeg_loop(self):
        ...
        while self._is_mirroring:
            try:
                async with aiohttp.ClientSession() as http:
                    async with http.get(url, timeout=aiohttp.ClientTimeout(total=None)) as resp:
                        boundary = None
