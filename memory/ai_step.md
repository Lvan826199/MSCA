...
## 2026-06-12 — 拉取最新代码并重新打包（第三次）

### 触发背景

用户要求拉取最新代码后按 Windows 打包流程打包。本次拉取包含 go-ios 更新（bin/ios/ios.exe 升级至 23MB）、wintun.dll 新增、goios_adapter 改进、backend-manager-state 改进、iOS tunnel 脚本、设备部署指南等，共 16 文件变更。

### 操作摘要

| 类别 | 操作 | 涉及范围 |
|:---|:---|:---|
| 代码拉取 | `git pull origin master`，`e8e4941..ddb8576`，16 文件变更 | — |
| 依赖同步 | `npm install`、`npm --prefix frontend install`、`uv sync` 全部 up to date | — |
| 后端编译 | `npm run backend:build` → Nuitka 4.1.2 / Python 3.13 编译成功，639 files 链接 | `dist/backend/msca-backend/` |
| 脚本修复 | `build-backend.mjs` 新增 `safeRemove()` 处理 Windows EPERM 锁定问题（空目录被 FS 句柄锁定） | `scripts/build-backend.mjs` |
| 后端验证 | `npm run backend:verify` → 资源 5/5，`/health` ok，`/api/devices` 200 | — |
| Electron 打包 | `npx electron-builder`（增量，产物已就绪）→ `MSCA Setup 0.1.0.exe` 144MB | `dist/electron/` |
...
| `/health` + `/api/devices` | ✅ |
| electron-builder 打包 | ✅ `MSCA Setup 0.1.0.exe` 生成（144MB） |

### 遇到的问题与修复

- `build-backend.mjs` 第 85 行 `rmSync` 对 `dist/backend/msca-backend` 报 EPERM：Windows 文件系统对空目录保持句柄锁定，UNC 路径 `\\?\` 格式触发权限拒绝。
- 修复方案：将 `rmSync` 包装为 `safeRemove()`，捕获 EPERM/EBUSY 后降级为覆盖模式（`cpSync` with `force: true`）。

### 最终提交 hash

- 代码基于 `ddb8576`（远程最新）；本次无业务代码变更，仅修复打包脚本
...
- `48d8a3e` fix(ios): 恢复 Web 端点击控屏兼容
- 投屏正常存档标签：`mirror-ok-20260507` → `f8ba767`。
...
- `ab6f3cc` fix(stability): 修复隐藏功能性风险
...
- `33450ef` fix(mirror): 修复投屏画面缩放与启动超时提示
...
- `cde3690` fix(ios): 增强 WDA 排障提示与发布文档
...
在 N3 短时稳定性验证通过后，继续按发布前回归计划执行 1 小时 Android+iOS 混合投屏长稳压测，补齐 N3 验收标准中“连续运行 1 小时无崩溃、停止后释放正常”的验证记录。

### 操作摘要

| 类别 | 操作 | 涉及范围 |
|:---|:---|:---|
| 长稳压测启动 | 使用安装版后端端口 `18002` 启动 Android `BH9504LNDQ` 与 iOS 15.1 `00008101-001859DE1E38001E` 投屏 | `/api/mirror/*/start` |
| 周期采样 | 每 5 分钟采样一次，共 12 轮，记录 `/health`、后端内存与 CPU | `msca-backend` 进程 |
| 停止释放验证 | 1 小时后停止 Android/iOS 投屏并检查最终健康状态 | `/api/mirror/*/stop`, `/health` |
| 文档同步 | 将 N3 长稳验收结果同步到计划与操作记录 | `doc/下一步计划.md`, `memory/ai_step.md` |

### 验证步骤（已执行）

1. **后端端口确认**：安装版后端端口 `18002`，`/health` 可访问。
2. **Android 投屏启动**：`BH9504LNDQ` → `started`，尺寸 `1440x2880`。
3. **iOS 15.1 投屏启动**：`00008101-001859DE1E38001E` → `started`，尺寸 `428x926`。
4. **1 小时健康采样**：12 轮采样均为 `health=ok`。
5. **内存观察**：后端内存从 `121225216` bytes 逐步下降并稳定在 `85004288` ~ `85766144` bytes 区间，未观察到持续增长。
6. **CPU 观察**：后端 CPU 累计值从 `23.453125` 增至 `134.78125`，符合持续运行采样过程中的正常累计增长。
7. **停止释放**：Android 与 iOS stop 均返回 `stopped`。
8. **最终健康检查**：停止后 `/health` → `ok`。

### 发现与处理

- N3 的 1 小时混合投屏稳定性验收已通过，未观察到后端崩溃、健康检查失败或内存持续增长。
- 停止后 Android/iOS 投屏接口均正常释放，最终健康检查仍为 `ok`。
- 下一阶段应进入 N4，重点不是继续压测，而是补齐 WDA 签名过期、未信任设备、端口占用和 adapter 启动失败等可定位日志与前端提示。

### 后续建议

- 执行 N4：梳理现有 iOS adapter/WDA 错误分支，增加更明确的错误分类、日志和前端展示文案。
- N4 完成后继续 N5：完善发布文档、操作手册和常见问题。

### 最终提交 hash

- `3af99c0` docs(release): 记录多设备长稳压测结果
...
- `7bac75d` docs(release): 记录多设备短时稳定性验证
...
## 2026-07-01 - 修复日志页切换仍会停止投屏

### 触发背景

用户继续测试时反馈：进入投屏后切换到运行日志页，投屏仍然会断开。后端日志确认在切换后出现控制 WebSocket 断开，并紧接着触发 `iOS 投屏已停止`。

### 操作摘要

| 类别 | 操作 | 涉及文件 |
|:---|:---|:---|
| 根因定位 | 通过后端日志确认切换日志页仍触发了投屏停止链路，说明上轮 `KeepAlive include="MirrorView"` 在异步路由组件场景下没有可靠命中缓存 | 后端运行日志 |
| 前端生命周期 | 将 `MirrorView` 从路由异步组件改为 `App.vue` 中的常驻组件：首次进入投屏页后挂载，后续切到日志/设置页仅 `v-show` 隐藏，不再卸载组件、不再触发后端 stop | `frontend/src/App.vue` |
| 路由调整 | `/mirror` 路由保留用于导航和 query，但组件改为空占位，避免 `MirrorView` 同时静态/动态导入导致构建警告 | `frontend/src/router/index.js` |
...
| `npm --prefix frontend run build` | 通过，且不再出现 `MirrorView` 同时静态/动态导入警告 |
| `npm run verify` | 通过，覆盖 `check:agents`、`lint:check`、前端 14 项单测、Electron 7 项单测、后端 62 项单测与前端构建 |

### 注意事项

- 当前设计下，“运行日志/设置/投屏监控”之间切换不会停止投屏；只有投屏页内的“返回”“全部停止”或单设备关闭按钮会释放后端投屏资源。
