...
| [设备部署指南](doc/设备部署指南.md) | iOS WDA 打包签名安装与 Android 设备连接准备 |
| [修复日志](doc/修复日志.md) | 全项目 bug 排查修复记录（根因、修复方式与验证结果） |
| [会话纪要-2026-06-12](doc/会话纪要-2026-06-12.md) | 第五~七轮会话工作记录与 Windows 端回归验证清单（跨设备交接） |
| [Windows 端回归验证清单](doc/Windows端回归验证清单.md) | 第五~七轮修复在 Windows 真机的逐项验证清单（可勾选记录结果） |
| [AI 操作步骤记录](memory/ai_step.md) | 历次 AI 开发操作记录与验证结果 |
...
- **Node.js** 18+
- **Python** 3.13+（3.14 已验证；Nuitka 编译需 `nuitka>=4.1.2`）
- **uv** 包管理器
...
npm run verify           # 日常一键验收（规则入口校验 + lint:check + 前/后端/Electron 单测 + 前端构建）
...
本项目采用“三层隔离”维护 AI 助手上下文：

1. **规则只维护一份**：`CLAUDE.md` 是唯一完整规则源，写所有项目规范、语言要求、提交规范、测试要求、文档同步策略等。`AGENTS.md` 只作为 Codex 入口，不复制规则正文，只要求 Codex 先读取并遵守 `CLAUDE.md`。
2. **共享记忆单独放**：`MEMORY.md` 用于跨会话、跨机器共享的项目记忆，只写非敏感内容，例如工作偏好、长期注意事项、项目特殊约定。
3. **本地工具配置不要同步**：`.claude/settings.local.json`、`.claude/worktrees/`、`.agent/`、`.agents/`、`.codex/`、`.Codex/` 只放本机工具私有内容，不入库、不跨工具同步；`.claude/` 下明确作为项目共享资产维护的 skills 可以继续入库。
...
- 提交涉及 `CLAUDE.md`、`AGENTS.md`、`scripts/check-agent-entry.mjs` 或 `package.json` 时，会自动执行 `node scripts/check-agent-entry.mjs`。
- 若检查失败，应保持 `AGENTS.md` 为短入口模板，不要把 `CLAUDE.md` 正文复制进去。
...
- **AndroidDriver**：通过 Python 后端 `ScrcpyServerManager` 推送并启动内置 scrcpy-server，处理 H.264 视频流与 scrcpy 控制协议。
- **IOSDriver**：封装 TideviceAdapter / GoIOSAdapter，管理 WDA 服务与 MJPEG 流。

### 多设备并发

- 桌面端 / Web 端：每个设备独立 WebSocket 连接与后端驱动实例。
...
- **桌面端**：Electron 主进程自动启动内嵌的 Python 后端（Nuitka 编译产物），从 18000 起自动探测本地可用端口并写入 `.backend-port`，完全离线可用。
- **Web 端**：部署公共后端服务（用户自有服务器），浏览器通过 HTTPS/WSS 连接远程后端。Web 端 TLS 由 Nginx 反向代理处理。

### 后端打包

- **工具**：Nuitka ≥4.1.2（Python → C 编译，生成独立可执行文件；产物名按平台区分：Windows 为 msca-backend.exe，Linux/macOS 为 msca-backend）
...
| **仅本地（local）** | 仅连接本地后端实际端口（从 `.backend-port` / Electron API 获取） | 离线环境 |
...
| 产物 | 说明 |
| :--- | :--- |
| **MSCA Setup 0.1.0.exe** | Electron 应用安装包，包含 Vue 前端、standalone 后端运行时和移动端二进制资源 |
| **dist/web/** | Vue 静态文件，部署至 Nginx 等 Web 服务器 |
| **dist/backend/msca-backend/** | Nuitka standalone 后端运行时目录，桌面端内嵌 / Web 端独立部署 |

## 重要注意事项

- **无需数据库**：所有设备状态为临时数据，配置使用 `electron-store`（桌面端）或 `localStorage`（Web 端）。
- **iOS 配对凭证**：首次连接 iOS 设备时自动生成 `selfIdentity.plist`（通常位于项目根目录，具体由 go-ios/tidevice 运行环境决定），包含配对密钥，已加入 `.gitignore`，禁止提交到远程仓库。
- **iOS WDA 签名**：WebDriverAgent 需使用有效开发者证书签名，否则 7 天过期。
- **未知版本 iOS 适配**：iOS 版本号缺失时优先使用 Tidevice，避免 iOS 15.x 设备误走 go-ios；若 Tidevice 不可用再回退 go-ios。
...
npm run verify
```
**预期结果**：`check:agents`、`lint:check`、前端/后端/Electron 单元测试与前端构建均通过。
...
- `electron:build` 会通过 `ELECTRON_BUILDER_BINARIES_MIRROR` 使用 electron-builder 二进制镜像；Windows 非管理员环境已关闭 `signAndEditExecutable`，避免解压 `winCodeSign` 符号链接权限失败。
- 日常开发优先执行步骤 0；如涉及后端启动链路，仍需执行步骤 2 做 `/health` 实测。
- 验证步骤 0、1 和 2 为必选项，步骤 3 在发布前执行，步骤 4 可在最终发布时执行。
- 所有验证通过后方可提交代码或发起 PR。
- **强制要求**：每次开发任务完成后，必须将本次操作行为、验证结果写入 [memory/ai_step.md](memory/ai_step.md)，保持操作记录可追溯。格式参考已有记录。

### AI 操作记录强制规则

> 以下规则必须严格执行，不得跳过。

1. **每次开发完成后**，必须将本次操作摘要、验证步骤与结果写入 `memory/ai_step.md`
2. **同步更新**：如有架构变更、新增模块、或者新的验证流程，需同步更新本文档 (`CLAUDE.md`)、`README.md`、`doc/下一步计划.md` 等对应文档
3. 记录内容至少包含：日期、触发背景、操作摘要表、验证结果、最终提交 hash
4. 本文档 (`CLAUDE.md`) 和 `memory/ai_step.md` 作为长期维护文档，不得删除或重置
